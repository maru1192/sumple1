...
def display_select_mode():
    ...
    回答モードのラジオボタンを表示
    """
    # サイドバー用に修正：st.columnsを削除
    # 「label_visibility="collapsed"」とすることで、ラジオボタンを非表示にする
    st.session_state.mode = st.radio(
        label="",
        options=[ct.ANSWER_MODE_1, ct.ANSWER_MODE_2],
        label_visibility="collapsed"  # サイドバーでは見やすくするため表示
    )
...
def display_initial_ai_message():
    ...
    AIメッセージの初期表示
    """
    # with st.chat_message("assistant"):をコメントアウト
    # 「社内文書検索」の機能説明
...
def display_message():
    ...
    with st.chat_message("assistant"):
        ...
        # 注意メッセージを追加（黄色い注意マーク付き）
        st.warning(":material/warning: 具体的に入力した方が期待通りの回答を得やすいです")
